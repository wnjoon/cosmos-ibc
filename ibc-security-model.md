...
클라이언트(Light Client)는 각 체인에서 고유하게 확인되는 Client ID로 식별된다. 이 클라이언트는 두 체인 간 연결을 유지하고, 연결되는 체인의 상태가 변하지 않고 일관된지 확인하는 중요한 역할을 한다.

일반 노드와는 달리, 클라이언트는 모든 트랜잭션과 블록 데이터를 저장하지 않고, 상대 체인의 상태를 검증하기 위한 [최소한의 요소](#클라이언트-상태-업데이트)를 확인하기 위해 해당 체인으로부터 새로운 블록이 생성될 때마다 해당 정보를 주기적으로 업데이트하고 검증한다. 이러한 요소에는 블록의 생성시간(timestamp), 트랜잭션이 포함된 블록의 루트 해시(root hash), 다음 블록의 검증자 집합 해시(next validator set hash)가 있다. 이를 통해 체인 간 통신이 안전하게 이루어지며, 연결된 체인의 상태가 변조되지 않았음을 보장할 수 있다.

[릴레이어](#릴레이어)는 상대 체인에서 새로운 블록이 생성되었다는 이밴트를 받아 클라이언트로 전달한다. 클라이언트는 전달받은 패킷을 내부에서 검증하여 상대 체인이 정상적이라는 것을 내부에서 판단한다. 이는 네트워크가 릴레이어의 신뢰성에 의존하지 않고, 모든 IBC 보안은 클라이언트에서 수행하는 검증 과정을 통해 보장되는 효과가 있다. 또한 클라이언트는 체인의 내부에 존재하기 때문에 체인의 보안이 네트워크 전체의 보안에 영향을 주게 되므로 안전한 IBC 네트워크 환경이 구성된다.
